...
{
	global $db;
	$sql = 'SELECT data_id FROM ' . TABLE_PERSONNEL_NAME . '_data WHERE group_name=\'level\' ORDER BY weight ASC';
	$result = $db->query( $sql );
	$weight = 0;
	while( $row = $result->fetch() )
	{
		++$weight;
		$db->query( 'UPDATE ' . TABLE_PERSONNEL_NAME . '_data SET weight=' . $weight . ' WHERE group_name=\'level\' AND data_id=' . $row['data_id'] );
	}
	$result->closeCursor();
}
...
if( ACTION_METHOD == 'delete' )
{
	...
	if( ! empty( $del_array ) )
	{
		...
		foreach( $del_array as $data_id )
		{
			...
			$db->query( 'DELETE FROM ' . TABLE_PERSONNEL_NAME . '_data WHERE group_name=\'level\' AND data_id = ' . ( int )$data_id );
...
elseif( ACTION_METHOD == 'status' )
{
	...
	{
		$sql = 'UPDATE ' . TABLE_PERSONNEL_NAME . '_data SET status=' . $new_vid . ' WHERE group_name=\'level\' AND data_id=' . $data_id;
		if( $db->exec( $sql ) )
		{
...
elseif( ACTION_METHOD == 'weight' )
{
	...
	if( $token == md5( $nv_Request->session_id . $global_config['sitekey'] . $data_id ) )
	{
		$sql = 'SELECT data_id FROM ' . TABLE_PERSONNEL_NAME . '_data WHERE group_name=\'level\' AND data_id !=' . $data_id . ' ORDER BY weight ASC';
		$result = $db->query( $sql );
		...
		while( $row = $result->fetch() )
		{
			...
			$sql = 'UPDATE ' . TABLE_PERSONNEL_NAME . '_data SET weight=' . $weight . ' WHERE group_name=\'level\' AND data_id=' . intval( $row['data_id'] );
			$db->query( $sql );
		}

		$sql = 'UPDATE ' . TABLE_PERSONNEL_NAME . '_data SET weight=' . $new_vid . ' WHERE group_name=\'level\' AND data_id=' . $data_id;
		if( $db->exec( $sql ) )
		{
		...
		if( empty( $error ) )
		{
			...
			if( $data['data_id'] == 0 )
			{
				try
				{
					...
					{
						nv_insert_logs( NV_LANG_DATA, $module_name, $lang_module['level_add'], 'data_id: ' . $data['data_id'], $admin_info['userid'] );
						
						$nv_Request->set_Session( $module_data . '_success', $lang_module['level_insert_success'] );
					}
					else
					{
			...
			else
			{
				try
				{
					...
					if( $stmt->execute() )
					{
						nv_insert_logs( NV_LANG_DATA, $module_name, $lang_module['level_edit'], 'data_id: ' . $data['data_id'], $admin_info['userid'] );
						$nv_Request->set_Session( $module_data . '_success', $lang_module['level_update_success'] );
					}
					else
					{
						$error['warning'] = $lang_module['level_error_save'];
