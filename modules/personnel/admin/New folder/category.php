...
function fix_category_order( $parent_id = 0, $order = 0, $lev = 0 )
{
	...
	$sql = 'SELECT category_id, parent_id FROM ' . TABLE_PERSONNEL_NAME . '_category WHERE parent_id=' . $parent_id . ' ORDER BY weight ASC';
	...
	foreach( $array_cat_order as $category_id_i )
	{
		++$order;
		++$weight;
		$sql = 'UPDATE ' . TABLE_PERSONNEL_NAME . '_category SET weight=' . $weight . ', sort=' . $order . ', lev=' . $lev . ' WHERE category_id=' . intval( $category_id_i );
		$db_slave->query( $sql );
	...
	if( $parent_id > 0 )
	{
		$sql = 'UPDATE ' . TABLE_PERSONNEL_NAME . '_category SET numsubcat=' . $numsubcat;
		if( $numsubcat == 0 )
		{
			$sql .= ",subcatid=''";
		}
...
if( ACTION_METHOD == 'delete' )
{
	...
	if( ! empty( $del_array ) )
	{
		...
		foreach( $del_array as $category_id )
		{
			...
			$db_slave->query( 'DELETE FROM ' . TABLE_PERSONNEL_NAME . '_category WHERE category_id = ' . ( int )$category_id );
			$db_slave->query( 'DELETE FROM ' . TABLE_PERSONNEL_NAME . '_category WHERE parent_id = ' . ( int )$category_id );
...
elseif( ACTION_METHOD == 'status' )
{
	...
	{
		$sql = 'UPDATE ' . TABLE_PERSONNEL_NAME . '_category SET status=' . $new_vid . ' WHERE category_id=' . $category_id;
		if( $db_slave->exec( $sql ) )
		{
			...
			$json['error'] = $lang_module['category_error_status'];	
...
elseif( ACTION_METHOD == 'inhome' )
{
	...
	if( $token == md5( $nv_Request->session_id . $global_config['sitekey'] . $category_id ) )
	{
		$sql = 'UPDATE ' . TABLE_PERSONNEL_NAME . '_category SET inhome=' . $new_vid . ' WHERE category_id=' . $category_id;
		if( $db_slave->exec( $sql ) )
		{
			nv_insert_logs( NV_LANG_DATA, $module_name, 'log_change_inhome_category', 'category_id:' . $category_id, $admin_info['userid'] );
			
			$nv_Cache->delMod($module_name);
			
			$json['success'] = $lang_module['category_inhome_success'];	
...
elseif( ACTION_METHOD == 'weight' )
{
	...
	if( $token == md5( $nv_Request->session_id . $global_config['sitekey'] . $category_id ) )
	{
		list( $category_id, $parent_id, $numsubcat ) = $db_slave->query( 'SELECT category_id, parent_id, numsubcat FROM ' . TABLE_PERSONNEL_NAME . '_category WHERE category_id=' . $category_id )->fetch( 3 );
		if( $category_id > 0 )
		{
 
			$sql = 'SELECT category_id FROM ' . TABLE_PERSONNEL_NAME . '_category WHERE category_id!=' . $category_id . ' AND parent_id=' . $parent_id . ' ORDER BY weight ASC';
			$result = $db_slave->query( $sql );
			...
			while( $row = $result->fetch() )
			{
				...
				$sql = 'UPDATE ' . TABLE_PERSONNEL_NAME . '_category SET weight=' . $weight . ' WHERE category_id=' . intval( $row['category_id'] );
				$db_slave->query( $sql );
			}

			$sql = 'UPDATE ' . TABLE_PERSONNEL_NAME . '_category SET weight=' . $new_vid . ' WHERE category_id=' . $category_id;
			if( $db_slave->exec( $sql ) )
			{
				fix_category_order();
...
elseif( ACTION_METHOD == 'add' || ACTION_METHOD == 'edit' )
{
	...
	if( $data['category_id'] > 0 )
	{
		...
		FROM ' . TABLE_PERSONNEL_NAME . '_category  
	...
	if( $nv_Request->get_int( 'save', 'post' ) == 1 )
	{
		...
		$stmt = $db_slave->prepare( 'SELECT COUNT(*) FROM ' . TABLE_PERSONNEL_NAME . '_category WHERE category_id !=' . $data['category_id'] . ' AND alias= :alias' );
		...
		{
			$parentid_alias = $db_slave->query( 'SELECT  FROM ' . TABLE_PERSONNEL_NAME . ' WHERE category_id=' . $data['parent_id'] )->fetchColumn();
		...
		if( empty( $error ) )
		{
			try
			{
				...
				if( $data['category_id'] == 0 )
				{
					...
					$stmt = $db_slave->prepare( 'SELECT max(weight) FROM ' . TABLE_PERSONNEL_NAME . '_category WHERE parent_id= ' . intval( $data['parent_id'] ) );
					$stmt->execute();
					$weight = $stmt->fetchColumn();

					$weight = intval( $weight ) + 1;

					$stmt = $db_slave->prepare( 'INSERT INTO ' . TABLE_PERSONNEL_NAME . '_category SET 
				...
				else
				{
					...
					$stmt = $db_slave->prepare( 'UPDATE ' . TABLE_PERSONNEL_NAME . '_category SET 
						parent_id = ' . intval( $data['parent_id'] ) . ', 
						inhome=' . intval( $data['inhome'] ) . ', 
						status=' . intval( $data['status'] ) . ', 
						date_modified=' . intval( $data['date_modified'] ) . ', 
					...
					if( $stmt->execute() )
					{
						...
						{
							$stmt = $db_slave->prepare( 'SELECT max(weight) FROM ' . TABLE_PERSONNEL_NAME . '_category WHERE parent_id= :parent_id ' );
							...
							$weight = intval( $weight ) + 1;
							$sql = 'UPDATE ' . TABLE_PERSONNEL_NAME . '_category SET weight=' . $weight . ' WHERE category_id=' . intval( $data['category_id'] );
							$db_slave->query( $sql );
	...
	$sql = 'SELECT category_id, title, lev FROM ' . TABLE_PERSONNEL_NAME . '_category WHERE category_id !=' . $data['category_id'] . ' ORDER BY sort ASC';
	...
	if( $array_inhome )
	{
		foreach( $array_inhome as $key => $item )
		{		
...
$sql = TABLE_PERSONNEL_NAME . '_category WHERE  parent_id = ' . $parent_id;
...
if( $parent_id > 0 )
{
	...
	while( $parentid_i > 0 )
	{
		list( $category_id_i, $parentid_i, $title_i ) = $db_slave->query( 'SELECT category_id, parent_id, title FROM ' . TABLE_PERSONNEL_NAME . '_category 
