...
<div id="content"> 
	...
	<div class="panel panel-default">
		<div class="panel-heading">	
			<h3 class="panel-title" style="float:left; text-transform:uppercase;font-weight:bold"><i class="fa fa-list"></i> {LANG.bookstatus_list}</h3> 
		...
		<div class="panel-body">
			<form action="#" method="post" enctype="multipart/form-data" id="form-bookstatus">
					...
					<table class="table table-bordered table-hover">
						<thead>
							<tr>
								...
								<td class="col-sm-12 text-left"><a {TITLE_ORDER} href="{URL_TITLE}"><strong>{LANG.bookstatus_title}</strong></a> </td>
								<td class="col-sm-4 text-left"><a {STATUS_ORDER} href="{URL_STATUS}"><strong>{LANG.bookstatus_status}</strong></a> </td>
						...
						<tbody>
							...
							<tr id="group_{LOOP.data_id}">
								...
								<td class="text-right">
									...
									<a href="javascript:void(0);" onclick="delete_bookstatus('{LOOP.data_id}', '{LOOP.token}')" data-toggle="tooltip" title="{LANG.delete}" class="btn btn-danger  btn-sm"><i class="fa fa-trash-o"></i>
								</td>
							</tr>
							 <!-- END: loop -->
						</tbody>
...
function delete_bookstatus(data_id, token) {
	if(confirm('{LANG.confirm}')) {
		$.ajax({
			url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=bookstatus&action=delete&nocache=' + new Date().getTime(),
			type: 'post',
			dataType: 'json',
			...
			success: function(json) {
				...
				if (json['error']) {
					$('#bookstatus-content').prepend('<div class="alert alert-danger"><i class="fa fa-exclamation-circle"></i> ' + json['error'] + '</div>');
				}
				
				if (json['success']) {
					$('#bookstatus-content').prepend('<div class="alert alert-success"><i class="fa fa-check-circle"></i> ' + json['success'] + '</div>');
...
$('.formajax').on('change', function() {
	...
	var id = $(this).attr('id');
	$.ajax({
		url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=bookstatus&nocache=' + new Date().getTime(),
		type: 'post',
		dataType: 'json',
