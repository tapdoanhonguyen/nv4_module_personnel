...
<div id="group-exam-content"> 
	...
	<div class="panel panel-default">
		<div class="panel-heading">
			<h3 class="panel-title" style="float:left"><i class="fa fa-list"></i> {LANG.premium_list}</h3> 
		...
		<div class="panel-body">
			<form action="#" method="post" enctype="multipart/form-data" id="form-premium">
				<div class="table-responsive">
					<table class="table table-bordered table-hover">
						<thead>
							<tr>
								<td class="text-center" style="width:80px" ><a {WEIGHT_ORDER} href="{URL_WEIGHT}"><strong>{LANG.weight}</strong></a></td>
								<td class="text-center"><a {TITLE_ORDER} href="{URL_TITLE}"><strong>{LANG.premium_title}</strong></a> </td>
								<td class="text-center"><strong>{LANG.premium_status} </strong></td>
								<td class="text-center"><strong>{LANG.action}</strong></td>
							</tr>
						</thead>
						<tbody>
							 <!-- BEGIN: loop --> 
							<tr id="group_{LOOP.premium_id}">
								<td class="text-center">
									<select id="id_weight_{LOOP.premium_id}" class="form-control btn-sm formajax" data-action="weight" data-id="{LOOP.premium_id}" data-token="{LOOP.token}">
									<!-- BEGIN: weight -->
								...
								<td class="text-center">
									<input name="status" id="id_status_{LOOP.premium_id}" value="1" type="checkbox" data-token="{LOOP.token}" data-premium_id="{LOOP.premium_id}" data-action="status" {LOOP.status_checked}>
								</td>
								<td class="text-center">
									<a href="{LOOP.edit}" data-toggle="tooltip" title="{LANG.edit}" class="btn btn-primary btn-sm"><i class="fa fa-pencil"></i></a>
									<a href="javascript:void(0);" onclick="delete_premium('{LOOP.premium_id}', '{LOOP.token}')" data-toggle="tooltip" title="{LANG.delete}" class="btn btn-danger btn-sm"><i class="fa fa-trash-o"></i>
								</td>
							</tr>
							 <!-- END: loop -->
						</tbody>
...
$('.formajax').on('change', function() {
	...
	var premium_id = $(this).attr('data-id');
	var new_vid = $(this).val();
	var id = $(this).attr('id');
	$.ajax({
		url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=premium&nocache=' + new Date().getTime(),
		type: 'post',
		dataType: 'json',
		data: 'action=' + action + '&premium_id=' + premium_id + '&new_vid=' + new_vid + '&token='+token,
		beforeSend: function() {
		...
		success: function(json) {
			...
			if ( json['new_vid'] == 0 || json['new_vid'] == 1){
				...
				$('#id_'+action+'_'+premium_id).val( json['new_vid'] );
...
$('input[name="status"]').on('change', function() {
	...
	var action = $(this).attr('data-action');
	var token = $(this).attr('data-token');
	var premium_id = $(this).attr('data-premium_id');
...
function delete_premium(premium_id, token) {
	if(confirm('{LANG.confirm}')) {
		$.ajax({
			url: script_name + '?' + nv_name_variable + '=' + nv_module_name + '&' + nv_fc_variable + '=premium&action=delete&nocache=' + new Date().getTime(),
			type: 'post',
			dataType: 'json',
			data: 'premium_id=' + premium_id + '&token=' + token,
			beforeSend: function() {
